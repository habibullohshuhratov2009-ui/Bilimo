  ...
  return (
    <main className="min-h-dvh bg-[#F8FAFC] text-[#0F172A] px-4 py-6">
      <div className="max-w-md mx-auto flex flex-col gap-4">
        ...
        <Card className="bg-gradient-to-br from-[#4F46E5] to-[#6D28D9] text-white border-0 p-5">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-indigo-200 text-sm font-semibold">Tangalarim</p>
              <div className="flex items-center gap-2 mt-1">
                <Coin size={40} />
                <span className="text-5xl font-extrabold tabular-nums">{Number(me.coins)}</span>
              </div>
            </div>
            <div className="text-center bg-white/15 rounded-2xl px-4 py-3">
              <div className="text-3xl">🔥</div>
              <div className="text-2xl font-extrabold tabular-nums">{myStreak}</div>
        ...
        <div className="flex flex-col gap-3">
          <Link
            href="/yechish"
            className="min-h-14 rounded-2xl bg-[#4F46E5] text-white font-bold text-lg flex items-center justify-center gap-2 shadow-[0_4px_0_#3730A3] active:shadow-none active:translate-y-1 transition-all hover:bg-[#4338CA]"
          >
            🧠 Masala yechish
          </Link>
          <Link
            href="/duel"
            className="min-h-14 rounded-2xl bg-[#FACC15] text-[#0F172A] font-bold text-lg flex items-center justify-center gap-2 shadow-[0_4px_0_#CA8A04] active:shadow-none active:translate-y-1 transition-all hover:bg-[#EAB308]"
          >
            ⚔️ Duelga chaqirish
          </Link>
          <button
            onClick={inviteFriend}
            className={`min-h-14 rounded-2xl font-bold text-lg flex items-center justify-center gap-2 transition-colors border-2 ${
              copied
                ...
                return (
                  ...
                  >
                    <span className="w-8 text-center text-lg font-extrabold">
                      {i < 3 ? MEDALS[i] : <span className="text-[#94A3B8] text-sm">{i + 1}</span>}
                    </span>
